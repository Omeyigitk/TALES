...
// Batch 7: Items 400-500
const batch7Raw = rawData.slice(400, 500);

const batch7Processed = batch7Raw.map(item => {
    let name_tr = item.name;

    // Magic Item Name Translations (401-500) - Focus on remaining Wondrous/Equip
    const translations = {
...
fs.writeFileSync('backend/data/items_batch_7.json', JSON.stringify(batch7Processed, null, 2));
console.log('Batch 7 processed.');
