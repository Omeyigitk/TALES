            ...
            <div className="max-w-7xl mx-auto px-4 pt-4">
                ...
                {(activeTab === 'attacks' || activeTab === 'spells') && (
                    <div className='pb-8 space-y-5'>
                        <div className='bg-gray-800 rounded-xl border border-blue-500/20 shadow-xl overflow-hidden p-6 text-center text-gray-400'>
                            Aksiyonlar ve Büyüler bu bölümde görüntülenir.
                        </div>
                    </div>
                )}

                {activeTab === 'inventory' && (
                    <div className='max-w-4xl space-y-8 pb-24 text-center'>
                        <div className='bg-gray-800 border border-gray-700 p-6 text-gray-300'>Envanter bölümü.</div>
                    </div>
                )}

                {activeTab === 'story' && (
                    <div className='max-w-4xl space-y-8 pb-24 text-center'>
                        <div className='bg-gray-800 border border-gray-700 p-6 text-gray-300'>Hikaye bölümü.</div>
                    </div>
                )}

                {toast.show && (
                    <div className={'fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] px-6 py-4 rounded-2xl border shadow-2xl flex items-center gap-4 ' + toast.color}>
                        <div className='flex-1 text-sm font-medium'>{toast.message}</div>
                        <button onClick={() => setToast(p => ({ ...p, show: false }))} className='opacity-50 hover:opacity-100 transition-opacity'>✕</button>
                    </div>
                )}
            </div>
        </div>
